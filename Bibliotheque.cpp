...
void Bibliotheque::afficheLivres() {
    ...
    for (int i = 0; i < nbrLivres; i++) {
        ...
        maxType = max(maxType, liste[i]->get_type_string().length());
    ...
    cout << left
         ...
         << setw(maxEtat + 2)     << "ETAT"
         << endl;
    size_t totalWidth = maxCode + maxTitre + maxAuteur + maxType + maxISBN + maxEtat + 12; // +2 par colonne
    ...
    for (int i = 0; i < nbrLivres; i++) {
        cout << left
             ...
             << setw(maxType + 2)     << liste[i]->get_type_string()
